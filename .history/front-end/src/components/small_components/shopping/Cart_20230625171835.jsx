...
export default function Cart() {
    ...
    const {getOrders} = useContext(LoginContext);
    ...
    useEffect(() => {
        const fetchOrders = async () => {
          try {
            setLoading(true);
            const orders = await getOrders(); // Assuming getOrders() is an asynchronous function that returns the orders
            const filteredOrders = orders.filter((el) => el.user_id === 1);
