...
const [formData, setFormData] = useState({
  product_name: '',
  product_quantity: 1,
  full_name:isLoggedIn?userData.firstname+ ' ' + userData.lastname:'' ,
  location: '',
...
useEffect(()=>{
  $(function(){
    ...
    setFormData({...formData , product_name:$(this).parents('.product-card').find('h4').text()})
...
/*   useEffect(() => {
  ...
  function ProductCard({product_name,price,img,id}) {
        ...
        <div  className='cards-sections d-grid justify-content-center '>
          ...
          <div style={{ gap:'1em 19em'}} className='cards-section-1 row-gap '>
            {paginate(prod.filter(el => el.category === ctg), currentPage, productsPerPage).map((item, i) => {
              const x = item.img;
              return (
                <div key={i} data-id={item.id}  className={`col product-card col rounded-3 shadow-sm`} style={{ width: '20em', height: '20em' ,userSelect:'none' }}>
                <h4 className='text-center fs-5 py-2 pb-1' style={{ letterSpacing: '1px' }}>{item.product_name} 
                </h4>
                
                <div className='d-flex pt-4'>
                  <img src={require(`../../../images/products/${ctg} (${x}).png`)} style={{ translate: '-35% 0' }} alt="" />
                  <h1 className='text-danger text-center d-flex flex-column' style={{ translate: '-20% 0', height: 'fit-content' }}  > <span> {item.price} </span> <span className='fs-4 opacity-75' style={{ translate: '0 -20%' }}>MAD</span> </h1>
                </div>
