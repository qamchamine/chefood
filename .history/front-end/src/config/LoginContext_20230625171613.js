...
const LoginProvider = ({ children }) => {
  ...
  const getOrders = async (setOrders)=>{ 
    const or = await axios.get('http://127.0.0.1:8000/api/order')
    .then((res) => {
...
getOrders(setOrders)
  ...
  return (
    <LoginContext.Provider value={{getOrders, isLoggedIn, login, logout, userData, setUserData, prod, setProd, loading, setLoading, formData, setFormData ,isLoggedFromOffers,setIsLoggedFromOffers,orders,setOrders}}>
      {children}
