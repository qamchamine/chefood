...
const LoginProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const storedProdOrder = localStorage.getItem('prodOrder');
    if (storedProdOrder) {
      storedProdOrder(JSON.parse(storedProdOrder));
    }
  }, []);
  useEffect(() => {
    localStorage.setItem('userData', JSON.stringify(userData));
    localStorage.setItem('prodOrder', JSON.stringify(prodOrder));
    localStorage.setItem('isLoggedIn', isLoggedIn);
  }, [userData, isLoggedIn,prodOrder]);
  ...
  const logout = () => {
    ...
    localStorage.removeItem('prodOrder');
